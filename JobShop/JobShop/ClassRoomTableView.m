...
@implementation ClassRoomTableView
@synthesize account;
...
-(void)callWebService{
    ...
    NSString *soapMessage = [NSString stringWithFormat:
                             ...
                             "<SOAP-ENV:Header/>\n"
                             "<S:Body>\n"
                             "<ns2:GetClassRoom xmlns:ns2=\"http://service.enterpriseApp.ld.org/\">\n"
                             "<account>"];
    soapMessage = [soapMessage stringByAppendingString:account];
    soapMessage = [soapMessage stringByAppendingString:@"</account>\n"];
    soapMessage = [soapMessage stringByAppendingString:@"</ns2:GetClassRoom>\n"];
    ...
    [theRequest addValue: @"http://service.enterpriseApp.ld.org/GetClassRoom"
...
-(void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string
{
    ...
    if(isSeparator){
        room = [[MyRoom alloc]init];
        room.room = [_receivedData objectAtIndex:0];
        room.clock = [_receivedData objectAtIndex:1];
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if( [elementName isEqualToString:@"ns2:GetClassRoomResponse"] ||
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ClassRoomTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
    ...
    MyRoom *roomItem = _roomList[indexPath.row];
    
    cell.roomLabel.text = roomItem.room;
...
{
    MyRoom *roomItem = _roomList[indexPath.row];
    userAccount.room = roomItem.room;
    ...
    [alert show];
     */
    [self performSegueWithIdentifier:@"getTable" sender:self];
}

-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
        RoomTableView *target = segue.destinationViewController;
